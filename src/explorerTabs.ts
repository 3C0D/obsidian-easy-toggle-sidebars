...
export async function goToExplorerTab(
  ...
  if (isLeftSplit) {
    ...
    if (isExplorerLeaf) {
      if (plugin.mouse.previousActiveSplitLeaf)
        await app.workspace.revealLeaf(plugin.mouse.previousActiveSplitLeaf);
    } else {
    ...
    plugin.mouse.previousActiveSplitLeaf = activeLeftSplit;
  }
}
